...
public class ImportantQuestionsParamTest {
//    public WebDriver driver = new FirefoxDriver();
    ...
    public ImportantQuestionsParamTest(String answer, String question, int index) {
        this.answer = answer;
...
@Parameterized.Parameters(name = "Текст вопроса: {0}, Текст ответа: {1}, Индекс элемента{2}")
public static Object[][] data(){
    ...
    public void checkAccordion(){
        ScooterMainPage objScooterPage = new ScooterMainPage(driver);
        objScooterPage.openMainPage();
        objScooterPage.clickCookieButton();
        objScooterPage.clickPanel(index);

        Assert.assertTrue(objScooterPage.isBlockVisible(index));
        assertEquals(answer, objScooterPage.getAnswerText(index));
        assertEquals(question, objScooterPage.getQuestionText(index));
    }
